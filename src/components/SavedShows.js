...
const SavedShows = () => {
    ...
    const deleteShow = async(itemId)=>{
        try {
            const res = movies.filter((item)=>item.id!==itemId)
  ...
  return (
    <div>
        ...
        <MdChevronLeft onClick={slideLeft} size={40} className='bg-white left-0 rounded-full absolute opacity-50 hover:opacity-100 cursor-pointer z-10 hidden group-hover:block'/>
            <div id={'slider'} className='h-ful w-full overflow-x-scroll whitespace-nowrap scroll-smooth scrollbar-hide relative'>
            {
                movies.map((item,id)=>{
                    return (
